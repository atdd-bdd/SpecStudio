...
QVector<PythonGenerator::GlueSig> PythonGenerator::collectGlueSigs(const SpectableFile& file, QStringList* conflicts)
{
    ...
    // Glue file (write fresh if absent; append missing stubs otherwise)
    {
        ...
        if (opts.overwriteGlue || !QFile::exists(gluePath)) {
            ...
            // A method the file declares that no step calls any more.
            {
                ...
                static const QRegularExpression decl(R"(^\s+def ([a-z]\w*)\(self)");
                for (const QString& dead : sourcescan::glueMethodsNoStepCalls(
                         gluePath, decl, expected, true))
